...
- Name: rahulshetty-login.spec.ts >> Login - Happy Path
- Location: tests\rahulshetty-login.spec.ts:9:5
...
Locator: div.card, .products, text=Products, text=Logout
Expected: visible
Error: Unexpected token "=" while parsing css selector "div.card, .products, text=Products, text=Logout". Did you mean to CSS.escape it?

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for div.card, .products, text=Products, text=Logout
...
```ts
  ...
  7  | const PASS = '123456';
  8  | 
  9  | test('Login - Happy Path', async ({ page }) => {
  10 |   // Navigate and login
  11 |   await page.goto(LOGIN_URL);
  12 |   await page.fill('#userEmail', USER);
  13 |   await page.fill('#userPassword', PASS);
  14 |   await page.click('#login');
  15 | 
  16 |   // Assertions
  17 |   await expect(page).not.toHaveURL(/\/auth\/login/);
> 18 |   await expect(page.locator('div.card, .products, text=Products, text=Logout')).toBeVisible({ timeout: 10000 });
     |                                                                                 ^ Error: expect(locator).toBeVisible() failed
  19 | });
  20 | 
```